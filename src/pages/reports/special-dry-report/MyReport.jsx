...
const SpecialDryReport = () => {
  const PAGE_COOKIE_KEY = "special-dry-report";
  ...
  const getData = useCallback((data) => {
    rpservice
      .getSpecialDry(data, { ignoreLoading: getIgnoreLoading() })
      .then((res) => {
        const { data } = res.data;
        setListDetail(data);
      })
      ...
      <Col span={12} style={{ paddingInline: 0 }}>
        <Flex gap={4} justify="end">
          ...
          <Button
            ...
            onClick={() => {
              const url = `${window.location.origin}/special-dry-report-print`;
  ...
  return (
    ...
    >
      <Space direction="vertical" size="middle" style={{ display: "flex", width: "100%", maxWidth: 1024 }}>
        {formSearch}
        <Card
          title={titleTable}
          style={{
        ...
        >
          <Table
            size="small"
            rowKey={(record, index) => `${record?.socode || "row"}-${record?.stcode || index}`}
            columns={columnDefs}
...
export default SpecialDryReport;